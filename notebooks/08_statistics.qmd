...
# Summary Measures and Frequencies

For continuous variables, common summary measures include the mean, median, standard deviation, variance, minimum, and maximum. Use `na.rm = TRUE` when missing values should be omitted:
...
For summaries by group, use `group_by()` and `summarize()`:
...
For categorical variables, `table()` gives counts and `prop.table()` gives proportions. `useNA = "ifany"` includes missing values when they occur:
...
The output of `summary()` also lets us know that some observations were dropped from the model because they had a missing value for at least one of the variables included in the model. Observations (rows) must have non-missing values for all variables in the model to be included in the estimation.
...
# BONUS MATERIAL: Quantiles

The median is the 50th percentile. Use `quantile()` to calculate several percentiles at once:
