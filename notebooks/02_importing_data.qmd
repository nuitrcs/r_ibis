...
RStudio Projects make relative file paths easier to use because the working directory is the project folder.

If you want to connect RStudio Projects with Git and GitHub, see RCDS's [Using Git with RStudio](https://rcdsdocs.it.northwestern.edu/tutorials/r/git-with-r.html) tutorial.
:::
...
We'll need to reference the column names frequently. To see those more compactly than with `str()`, use `names()`
...
# BONUS MATERIAL: Importing Excel Files

R can import Excel files (`.xls`, `.xlsx`) directly with the `readxl` package. Because we only need one function here, we can write the package name before the function instead of loading the package with `library()`.

```{r}
readxl::read_excel("data/NUdates.xlsx")
```

By default, `read_excel()` imports the first sheet. Specify a sheet by name when needed:

```{r}
readxl::read_excel("data/NUdates.xlsx", sheet = "holidays")
```

To use imported data later, assign it to a variable:

```{r, eval=FALSE}
nu_calendar <- readxl::read_excel("data/NUdates.xlsx")
```

Unlike `read.csv()`, `read_excel()` recognizes spreadsheet date/time cells and imports them as date-time values automatically.
