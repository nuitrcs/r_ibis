...
# restore the original name before the next example
names(healthdata)[2] <- "survey_year"
```

You can change multiple values at once as well:
...
# restore the original names before the dplyr examples
names(healthdata)[2:3] <- c("survey_year", "gender")
```
...
# change it back to kilograms before continuing
healthdata$weight <- healthdata$weight / 2.2
```
...
Change height back to centimeters by multiplying by 2.54, and convert the id variable back to an integer type.
...
# or
healthdata <- mutate(healthdata, bp_sys1 = ifelse(is.na(bp_sys1), 9999, bp_sys1))
```
...
# BONUS MATERIAL: More Data-Frame Changes

`mutate()` can change an existing column or create a new one. Save the result only when you intend to keep the change:

```{r}
# return a modified copy without changing healthdata
mutate(healthdata, weight_lbs = weight * 2.2)

# save a new column in healthdata
healthdata <- mutate(healthdata, height_inches = height / 2.54)
```

To remove rows or columns, create the subset you want and assign it to a new object or back to the original data frame. Avoid keeping many nearly identical copies of a dataset in the Environment.
