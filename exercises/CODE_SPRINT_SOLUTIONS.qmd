...
Convert `rain_avg` from inches to millimeters (multiply by 25.4).
...
Run a regression predicting `co2_growth_rate` as a function of `co2_per_capita`, `forest_land_pct`, `agriculture_land_pct`, `temp_avg`, and `rain_avg`.
...
```{r}
lm(co2_growth_rate ~ co2_per_capita + forest_land_pct + agriculture_land_pct + temp_avg + rain_avg,
   data=govenv) %>%
  summary()
