...
latin_america <- govenv$region == "Latin America"
maxval <- max(govenv$co2_per_capita[latin_america], na.rm=TRUE)
govenv$country[latin_america & govenv$co2_per_capita == maxval]
...
Convert `rain_avg` from millimeters to inches (divide by 25.4).
...
# alternative
govenv <- mutate(govenv, rain_avg = rain_avg / 25.4)
...
govenv %>%
  group_by(democracy) %>%
  summarize(mean_global = mean(globalization),
...
The visualization exercises use the original variables, so start this section with a fresh copy of the data. This also makes the solutions below runnable on their own.
...
# Alternative: zoom in without dropping observations outside the displayed range
ggplot(govenv, aes(x=forest_land_pct, y=rain_avg)) + 
  geom_point() +
...
The statistics exercises also use the original variables, so start with a fresh copy of the data.
